# Script objective: compute 7DADM impairment (I think?), where 7DADM is 7 day
# average daily maximum (I think?) which includes three days prior and theree
# days after a given day (check this with Jess again, or with notes)
...
base_loc = "data/" # Base folder for all data
fall_data_loc <- paste0(base_loc, "2023_fall_data_qcd/") # Contained subfolder with QC data from fall 2023
sum_data_loc <- paste0(base_loc, "2024_summer_data_qcd/") # Contained subfolder with QC data from summer 2024
lookup_loc = base_loc
...
# ASK JESS: After running following code chunk, got this warning message twice:
...
# Applied function to exclude JimsWell site maybe?
...
file_no = 0 # For iteration number
for(this_file in data_files){ # for each file,
  # update file_no counter
  file_no = file_no + 1 # Initiate iteration loop tracker
...
# I believe these are exceedences for quality limits or something that Jess mentioned?
...
for(site in unique(temp_stats$SiteName)){
  # What does comment below mean? Why is it there? Troubleshooting purposes?
  # cat(site, fill = TRUE)
...
# Note from Kaylee: besides the error messages, seems like everything is working
# as intended.
