...
for (this_file in filenames) {
  i = i+1
  #this_file = filenames[1] # for troubleshooting the for-loop
  cat(paste0("Reading file ", i, " of ", length(filenames), ": ", this_file), fill = TRUE)
  ...
  info_from_filename = unlist(strsplit(this_file, split="[_.]"))[1:4]
  ...
  if(deploy_season != "sum" & deploy_season != "fall"){
    stop("filename does not say sum or fall for the deployment season.")
  }
  deploy_year = 2000 + as.integer(info_from_filename[4]) # or we can format as character

  this_data <- read.csv(
    file = paste0(cropped_loc, this_file), as.is=T, skip=1, fill=T, header=F
  ...
  combined <- dplyr::bind_rows(combined, this_data)  ## Adds the datafile's data to the existing combined datafile.
...
if("air" %in% unique(combined$media)){
  combined <- combined %>%
    ...
    dplyr::mutate(
      AWMaxDiff = dailymax_air - dailymax_water,
  ...
  combined <- combined %>%
    ...
    #pivot longer for plotting purposes later; to plot on same graph, need "calc" column ("grouping variable")
    tidyr::pivot_longer(
      cols = dailymin_water:WaterRange,
      names_to = "calc",
      values_to = "value"
    )
}
...
for(s in sites){
  ...
  rangeplot <- ggplot2::ggplot(
    ...
    ggplot2::labs(
      title = paste0(s," Air and Water Temperature ranges"),
      x = "Date",
      y = "Temperature (C)",
      color = "Medium"
    ) +
  ...
  ggplot2::ggsave(
    paste0(qc_plots_loc, s, "_AirWaterTempRange.png"),
    rangeplot,
    width = 11,
    height = 8.5,
    units = "in"
  ...
  maxdiffplot <- ggplot2::ggplot(
    ...
    ggplot2::geom_line(na.rm=TRUE) +
    ggplot2::labs(
      title = paste0(s," Max Air and Water Temperature and Difference"),
      x = "Date",
      y = "Temperature (C)",
      color = "Medium"
    ) +
  ...
  ggplot2::ggsave(
    paste0(qc_plots_loc, s, "_MaxDiffAirWaterTemp.png"),
    maxdiffplot,
    width = 11,
    height = 8.5,
    units = "in"
  ...
  htmlwidgets::saveWidget(
    maxdifplotly,
    paste0(qc_plots_loc, s, "_MaxDiffAirWaterTemp.html")
  )
  htmlwidgets::saveWidget(
    rangeplotly,
    paste0(qc_plots_loc, s, "_AirWaterTempRange.html")
  )
}; cat("Done.", fill = TRUE)
